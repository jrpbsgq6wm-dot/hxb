...
void fpga_interface_init(void)
{
	...
	DBG("test 3\n");
	uio_init(&uio_share_mem_IQ); 
	DBG("test 4\n");
	Debug("uio_share_mem_IQ.mem_ptr : %p\n", uio_share_mem_IQ.mem_ptr);
	uio_init(&uio_sensor_mem_0); 
	DBG("test 5\n");
	uio_sensor_mem_1.mem_ptr = (int *)((char *)(uio_sensor_mem_0.mem_ptr) + 0x100000); 
	Debug("uio_sensor_mem_1.mem_ptr : %p\n", uio_sensor_mem_1.mem_ptr);
	uio_sensor_mem_2.mem_ptr = (int *)((char *)(uio_sensor_mem_0.mem_ptr) + 0x200000);
	Debug("uio_sensor_mem_2.mem_ptr : %p\n", uio_sensor_mem_2.mem_ptr); 
	uio_sensor_mem_3.mem_ptr = (int *)((char *)(uio_sensor_mem_0.mem_ptr) + 0x300000);
	Debug("uio_sensor_mem_3.mem_ptr : %p\n", uio_sensor_mem_3.mem_ptr);
	uio_sensor_mem_4.mem_ptr = (int *)((char *)(uio_sensor_mem_0.mem_ptr) + 0x400000);
	Debug("uio_sensor_mem_4.mem_ptr : %p\n", uio_sensor_mem_4.mem_ptr);
	//uio_init(&uio_sensor_mem_1); 
	//  DBG("test 6\n");
	//uio_init(&uio_sensor_mem_2); 
	// DBG("test 7\n");
	// uio_init(&uio_sensor_mem_3); 
	// DBG("test 8\n");
	// uio_init(&uio_sensor_mem_4); 
	// DBG("test 9\n");
	uio_init(&uio_sensor_mem_8);
	uio_init(&uio_share_mem_original); 
	uio_sensor_mem_9.mem_ptr = (int *)((char *)(uio_sensor_mem_8.mem_ptr) + 0x100000); 
	Debug("uio_sensor_mem_9.mem_ptr : %p\n", uio_sensor_mem_9.mem_ptr);
	uio_sensor_mem_A.mem_ptr = (int *)((char *)(uio_sensor_mem_8.mem_ptr) + 0x200000);
	Debug("uio_sensor_mem_A.mem_ptr : %p\n", uio_sensor_mem_A.mem_ptr);
	uio_sensor_mem_B.mem_ptr = (int *)((char *)(uio_sensor_mem_8.mem_ptr) + 0x300000);
	Debug("uio_sensor_mem_B.mem_ptr : %p\n", uio_sensor_mem_B.mem_ptr);
	uio_sensor_mem_C.mem_ptr = (int *)((char *)(uio_sensor_mem_8.mem_ptr) + 0x400000); 
	Debug("uio_sensor_mem_C.mem_ptr : %p\n", uio_sensor_mem_C.mem_ptr);
	// uio_init(&uio_sensor_mem_9); 
	//  DBG("test 11\n");
	// uio_init(&uio_sensor_mem_A); 
	// DBG("test 12\n");
	//  uio_init(&uio_sensor_mem_B);
	//  DBG("test 13\n"); 
	// uio_init(&uio_sensor_mem_C);
	// DBG("test 14\n");  
...
//	gpio_set_value(938, 0); //低电平
	gpio_export(489);  //打开端口938
	gpio_set_dir(489, 1); //端口938为输出模式
	gpio_set_value(489, 0); //低电平
	usleep(10);
}
...
//	gpio_set_value(939, 1); //高电平
	gpio_export(490);  //打开端口938
	gpio_set_dir(490, 1); //端口938为输出模式
	gpio_set_value(490, 1); //高电平
	usleep(10);
}
...
//	gpio_set_value(934, 1); //高电平
	gpio_export(507);  //打开端口938
	gpio_set_dir(507, 1); //端口938为输出模式
	gpio_set_value(507, 1); //高电平
	usleep(10);
}
