...
void interface_selection(work_parameter_t work_mode){
    ...
    /* SVS */
    if (work_mode.SVS_mode == 0x00)                             //干端
    {
        /* SVS 干端输入：SVS 可用，SYNC 不可用 */
        signal_monitor_sync_enable(0);
        //判断输入为232/485
        SVS_set_input_mode((SVS_input_mode_t)work_mode.SVS_comm_mode);
        signal_monitor_enable(SIGNAL_MONITOR_SVS, 1);
    } else if (work_mode.SVS_mode == 0x01) {                    //湿端
        /* SVS 湿端输入：SVS 干端不检测，SVS 灯蓝色，SYNC 可用 */
