...
int fputc(int ch, FILE *f)
{
    ...
    (void)f;
    (void)HAL_UART_Transmit(&g_uart1_handle, &data, 1U, HAL_MAX_DELAY);
    return ch;
}
...
/**
 * @brief  通过 USART6 调试串口发送一段数据。
 * @note   发送逻辑统一收敛到 HAL，业务模块不再直接访问 SR、DR 寄存器。
 */
HAL_StatusTypeDef usart_send(const uint8_t *data, uint16_t length, uint32_t timeout)
{
    ...
    return HAL_UART_Transmit(&g_uart1_handle, (uint8_t *)data, length, timeout);
}
