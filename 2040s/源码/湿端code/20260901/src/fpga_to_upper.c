...
void lookfor_five_sensor_num(const int *ptr_sensor1,const int *ptr_sensor2,const int *ptr_sensor3,const int *ptr_sensor4,const int *ptr_sensor5)
{
    ...
    for (m=0;m<100;m++)
    {
        if ((*(ptr_sensor1+m*64)==0xDDDDDDDD)&&(*(ptr_sensor1+m*64+1)==(ptr_fpga_frame_first->FrameNumber-1)))
        {
            sensor1_num++;
        }
        if ((*(ptr_sensor2+m*64)==0xDDDDDDDD)&&(*(ptr_sensor2+m*64+1)==(ptr_fpga_frame_first->FrameNumber-1)))
        {
            sensor2_num++;
        }
        if ((*(ptr_sensor3+m*64)==0xDDDDDDDD)&&(*(ptr_sensor3+m*64+1)==(ptr_fpga_frame_first->FrameNumber-1)))
        {
            sensor3_num++;
        }
        if ((*(ptr_sensor4+m*64)==0xDDDDDDDD)&&(*(ptr_sensor4+m*64+1)==(ptr_fpga_frame_first->FrameNumber-1)))
        {
            sensor4_num++;
        }
        if ((*(ptr_sensor5+m*64)==0xDDDDDDDD)&&(*(ptr_sensor5+m*64+1)==(ptr_fpga_frame_first->FrameNumber-1)))
        {
            sensor5_num++;
        }
    }
...
void read_fpga_send_to_upper(void)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            else if ((ptr_fpga_frame_first->FrameHead == 0xAAAAAAAA) && (ptr_fpga_frame_first->SmallFrameHead == 0xBBBBBBBB))//原始数据
            {
                prepare_original_data_len();
                my_copy((unsigned char *)(ddr_sonar_data + SONAR_DATA_OFFSET), (unsigned char *)(uio_share_mem_original.mem_ptr), ptr_send_to_upper_package_first->SonarDataLength); 
                copy_sensor_data_from_pingpang_buf();
                copy_tail_to_sonar_data();
                send_all_package_to_upper_and_clear_sensor_num();
            } 
