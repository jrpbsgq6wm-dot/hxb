...
int parsing_instructions_200k(const RECV_UPPER_CONFIG_PARAMETERS *ptr_recv_upper_package, FPGA_CONFIG_PARAMETERS *ptr_fpga_config_para)
{
	...
	memset(ptr_fpga_config_para->pwm_registers_value.pwm_ip,0,sizeof(ptr_fpga_config_para->pwm_registers_value.pwm_ip));
	ptr_fpga_config_para->pwm_registers_value.pwm_ip[1] = 
    ((uint32_t)(unsigned char)ptr_recv_upper_package->UART_REQ) |          	  // byte 0
    ((uint32_t)((ptr_recv_upper_package->MCP4725_DAC >> 8) & 0xFF) << 8) |    // byte 1: b的高8位
    ((uint32_t)(ptr_recv_upper_package->MCP4725_DAC & 0xFF) << 16);           // byte 2: b的低8位
...
{
    int m;
	memset(sensor_mem_0.mem_ptr, 0, get_memory_size(sensor_mem_0.sysfs_path_file));
	memset(sensor_mem_1.mem_ptr, 0, get_memory_size(sensor_mem_1.sysfs_path_file));
}
...
int set_fpga_work_parameter(void){
	...
	len_recv_crc=recv_socket(Connect_fd,(char *)&recv_crc,sizeof(recv_crc));
	if (len_recv_crc < 0)
	{
		error_process();
		return -1;
	...
	//下发TVG数据
	memset(tvg_register_mem.mem_ptr, 0, SIZE_OF_TVG_MAX); //剩余tvg置0 
	memcpy((char *)tvg_register_mem.mem_ptr, (char *)&cmd_package.tvgGain, fpga_register_data.fpga_registers_200k.dac_registers_value.dac_sn*4);//下发tvg数据            
	//参数更新中断
	ptr_fpga_register_data->set_pr=1;
	usleep(1000);//1ms 
	ptr_fpga_register_data->set_pr=0;
